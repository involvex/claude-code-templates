...
async function monitorWorkerHealth(workerUrl: string): Promise<boolean> {
  ...
  try {
    ...
    if (response.ok || response.status === 405) {
      ...
      logWithTimestamp(
        `⚠️  Worker returned: ${response.status} ${response.statusText}`,
        "WARNING",
      );
      return false;
    }
...
async function monitorSandboxExecution(
  ...
  try {
    ...
    logWithTimestamp(
      `✅ Sandbox execution completed in ${duration}ms`,
      "SUCCESS",
    );
    logWithTimestamp(
      `   Exit code: ${result.success ? "0 (success)" : "1 (failed)"}`,
    );
    logWithTimestamp(
      `   Output length: ${result.output?.length || 0} characters`,
    );

    if (result.error) {
      logWithTimestamp(
        `   Error output: ${result.error.length} characters`,
        "WARNING",
      );
    }
    ...
    const errorMsg = error instanceof Error ? error.message : String(error);
    logWithTimestamp(
      `❌ Sandbox execution failed after ${duration}ms: ${errorMsg}`,
      "ERROR",
    );
    throw error;
  }
...
function displayMetrics(metrics: MonitoringMetrics) {
  ...
  console.log(
    `${colors.cyan}Total Execution Time:${colors.reset} ${metrics.executionTime}ms`,
  );
  console.log(
    `${colors.cyan}  ├─ Code Generation:${colors.reset} ${metrics.codeGenerationTime}ms`,
  );
  console.log(
    `${colors.cyan}  └─ Sandbox Execution:${colors.reset} ${metrics.sandboxExecutionTime}ms`,
  );

  if (metrics.memoryUsage) {
    console.log(
      `${colors.cyan}Memory Usage:${colors.reset} ${metrics.memoryUsage}MB`,
    );
  }
  ...
  if (metrics.errorDetails) {
    console.log(
      `${colors.red}Error Details:${colors.reset} ${metrics.errorDetails}`,
    );
  }
...
function displaySystemInfo() {
  ...
  console.log(
    `${colors.cyan}Node.js Version:${colors.reset} ${process.version}`,
  );
...
async function enhancedSandboxMonitoring(
  ...
  try {
    ...
    if (!workerHealthy) {
      logWithTimestamp(
        "⚠️  Worker is not healthy, attempting to continue...",
        "WARNING",
      );
    }
    ...
    const { code, duration: codeGenDuration } = await monitorCodeGeneration(
      anthropic,
      prompt,
    );
    ...
    const { result, duration: execDuration } = await monitorSandboxExecution(
      workerUrl,
      prompt,
    );
    ...
    logWithTimestamp(
      `❌ Monitoring session failed: ${metrics.errorDetails}`,
      "ERROR",
    );
    return false;
  }
...
async function main() {
  ...
  if (args.length < 1) {
    ...
    console.log(
      '  node monitor.ts "Sum array" YOUR_KEY https://your-worker.workers.dev',
    );
    console.log("");
    console.log("Environment Variables:");
    console.log("  ANTHROPIC_API_KEY - Anthropic API key");
    console.log(
      "  CLOUDFLARE_WORKER_URL - Worker endpoint (default: http://localhost:8787)",
    );
    console.log("");
    console.log(
      "This tool provides enhanced monitoring and debugging for Cloudflare sandbox operations.",
    );
    process.exit(1);
  }
  ...
  const workerUrl =
    args[2] || process.env.CLOUDFLARE_WORKER_URL || "http://localhost:8787";
  ...
  printSeparator("=", 70);
  console.log(
    `${colors.bright}${colors.cyan}     🎬 CLOUDFLARE SANDBOX MONITOR${colors.reset}`,
  );
  printSeparator("=", 70);
  console.log("");

  const success = await enhancedSandboxMonitoring(
    prompt,
    anthropicApiKey,
    workerUrl,
  );
