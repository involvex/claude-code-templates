...
describe("IntegrityValidator", () => {
  ...
  const testRegistryPath = path.join(
    process.cwd(),
    ".claude/security/component-hashes.json",
  );
...
Content`,
  ...
  describe("Hash Registry", () => {
    it("should update registry when requested", async () => {
      ...
      expect(registry[normalizedPath].hash).toBe(
        validator.generateHash("test content"),
      );
    ...
    it("should handle new components not in registry", async () => {
      ...
      expect(
        result.info.some((i) => i.code === "INT_I005" || i.code === "INT_I006"),
      ).toBe(true);
    });
  });
