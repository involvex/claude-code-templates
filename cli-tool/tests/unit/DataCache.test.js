...
describe("DataCache", () => {
  ...
  describe("getFileContent", () => {
    ...
    it("should handle file read errors", async () => {
      ...
      await expect(dataCache.getFileContent(mockFilePath)).rejects.toThrow(
        "File not found",
      );
    });
  ...
  describe("getCachedTokenUsage", () => {
    ...
    it("should compute and cache token usage", async () => {
      const result = await dataCache.getCachedTokenUsage(
        mockFilePath,
        mockComputeFunction,
      );
    ...
    it("should handle computation errors", async () => {
      ...
      await expect(
        dataCache.getCachedTokenUsage(mockFilePath, errorFunction),
  ...
  describe("invalidateFile", () => {
    ...
    it("should remove all cache entries for a file", async () => {
      // Populate caches
      dataCache.caches.fileContent.set(mockFilePath, {
        content: "test",
        timestamp: Date.now(),
      });
      dataCache.caches.parsedData.set(mockFilePath, {
        data: "test",
        timestamp: Date.now(),
      });
      dataCache.caches.computationResults.set(`tokens_${mockFilePath}`, {
        result: "test",
        timestamp: Date.now(),
      });
      ...
      expect(
        dataCache.caches.computationResults.has(`tokens_${mockFilePath}`),
      ).toBe(false);
    });
  });
