...
async function createClaudeConfig(options = {}) {
  ...
  if (
    ...
    console.log(
      chalk.red('❌ Error: --tunnel can only be used with --analytics, --chats, or --chats-mobile')
    );
  ...
  if (options.yes) {
    ...
    const selectedLanguage =
      options.template || options.language || projectInfo.detectedLanguage || 'common';
  ...
  if (config.language !== 'common') {
    console.log(
      chalk.yellow(`💡 Language-specific features for ${config.language} have been configured`)
    );
  }

  if (config.framework !== 'none') {
    console.log(
      chalk.yellow(`🎯 Framework-specific commands for ${config.framework} are available`)
    );
  }
...
async function installIndividualAgent(agentName, targetDir, options) {
  ...
  try {
    ...
    const agentsDir = path.join(scopeInfo.targetDir, scopeInfo.claudeSubdir, 'agents');
    ...
    if (!options.silent) {
      ...
      console.log(chalk.cyan(`📁 Installed to: ${path.relative(scopeInfo.targetDir, targetFile)}`));
...
async function installIndividualCommand(commandName, targetDir, options) {
  ...
  try {
    ...
    const commandsDir = path.join(scopeInfo.targetDir, scopeInfo.claudeSubdir, 'commands');
...
async function installIndividualMCP(mcpName, targetDir, options) {
  ...
  try {
    ...
    if (!options.silent) {
      ...
      console.log(
        chalk.cyan(
          `📁 Configuration merged into: ${path.relative(scopeInfo.targetDir, targetMcpFile)}`
        )
      );
      console.log(chalk.cyan(`📦 Downloaded from: ${githubUrl}`));
    }
  ...
  try {
    ...
    for (const installLocation of installLocations) {
      ...
      Object.keys(settingConfig).forEach((key) => {
        if (
          ...
          JSON.stringify(existingConfig[key]) !== JSON.stringify(settingConfig[key])
        ) {
      ...
      if (Object.keys(additionalFiles).length > 0) {
        console.log(
          chalk.blue(`📄 Installing ${Object.keys(additionalFiles).length} additional file(s)...`)
        );
      ...
      if (!options.silent) {
        console.log(
          chalk.green(`✅ Setting "${settingName}" installed successfully in ${installLocation}!`)
        );
    ...
    if (!options.silent) {
      if (successfulInstallations === installLocations.length) {
        ...
        console.log(
          chalk.red(`❌ ${failedCount} installation(s) failed due to permission or other errors.`)
        );
      }
    }
  ...
  try {
    ...
    for (const installLocation of installLocations) {
      ...
      if (await fs.pathExists(actualTargetFile)) {
        existingConfig = await fs.readJson(actualTargetFile);
        console.log(
          chalk.yellow(`📝 Existing ${settingsFile} found, merging hook configurations...`)
        );
      }
        ...
        Object.keys(hookConfig.hooks).forEach((hookType) => {
          if (!mergedConfig.hooks[hookType]) {
            ...
            if (Array.isArray(hookConfig.hooks[hookType])) {
              ...
              mergedConfig.hooks[hookType] = mergedConfig.hooks[hookType].concat(
      ...
      if (!options.silent) {
        console.log(
          chalk.green(`✅ Hook "${hookName}" installed successfully in ${installLocation}!`)
        );
        console.log(chalk.cyan(`📁 Configuration merged into: ${actualTargetFile}`));
        console.log(chalk.cyan(`📦 Downloaded from: ${githubUrl}`));
      }
...
function resolveScopeDirectory(scope, targetDir, componentType) {
  ...
  if (!scopeMap[normalizedScope]) {
    throw new Error(`Invalid scope: ${scope}. Valid: user, global, project, local`);
  }
...
function getEnterpriseDirectory() {
  ...
  const enterpriseMap = {
    ...
    win32: path.join(process.env.PROGRAMDATA || 'C:\\ProgramData', 'ClaudeCode'),
  };
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        if (responseText.includes('rate limit')) {
          ...
          console.log(
            chalk.yellow('💡 Install locally with: npm install -g claude-code-templates')
          );
...
async function installIndividualSkill(skillName, targetDir, options) {
  ...
  try {
    ...
    const skillsDir = path.join(scopeInfo.targetDir, scopeInfo.claudeSubdir, 'skills');
...
async function installMultipleComponents(options, targetDir) {
  ...
  try {
    ...
    if (options.scope) {
      ...
      // Map 'global' to 'user' (alias)
      const normalizedScope = normalizedScopeInput === 'global' ? 'user' : normalizedScopeInput;
      scopeLocations = [normalizedScope];

      if (normalizedScope === 'enterprise') {
        console.log(chalk.yellow('⚠️  Enterprise scope requires administrator privileges'));
      }

      console.log(chalk.cyan(`📍 Installing all components to scope: ${normalizedScope}`));
    }
    ...
    if (hasSettingsOrHooks && !options.yes && !scopeLocations) {
      ...
      console.log(
        chalk.cyan(`📋 Using --scope for configuration components: ${scopeLocations.join(', ')}`)
      );
    }
    ...
    for (const command of components.commands) {
      ...
      const commandSuccess = await installIndividualCommand(command, targetDir, {
        ...options,
        silent: true,
        scope: scopeLocations ? scopeLocations[0] : undefined,
      });
    ...
    if (options.yaml) {
      ...
      try {
        ...
        console.log(
          chalk.green(`✅ Workflow YAML saved: ${path.relative(targetDir, workflowFile)}`)
        );
      } catch (yamlError) {
    ...
    if (successfullyInstalled === totalComponents) {
      ...
      console.log(
        chalk.red(`❌ ${totalComponents - successfullyInstalled} component(s) failed to install.`)
      );
    } else {
    ...
    if (options.yaml) {
      ...
      console.log(
        chalk.cyan(`🚀 Use the workflow file with Claude Code to execute the complete setup`)
      );
    }
...
async function installWorkflow(workflowHash, targetDir, options) {
  ...
  try {
    ...
    console.log(
      chalk.cyan(`🚀 Use the workflow file with Claude Code to execute the complete workflow`)
    );
...
async function installComponentFromWorkflow(componentData, type, targetDir, options) {
  try {
    ...
    console.error(
      chalk.red(`❌ Error installing ${type} "${componentData.name}": ${error.message}`)
    );
    throw error;
  }
}
...
${workflowData.steps
  .map((step, index) => `# ${index + 1}. ${step.description} (${step.type}: ${step.name})`)
  .join('\n')}
#
# Total estimated time: ${Math.ceil(workflowData.steps.length * 2)} minutes
# Components required: ${workflowData.steps.filter((s) => s.type === 'agent').length} agents, ${
...
async function executeSandbox(options, targetDir) {
  ...
  if (!prompt) {
    ...
    console.log(chalk.cyan('═══════════════════════════════════════'));
    console.log(
      chalk.gray('Describe what you want to create in detail. The more specific you are,')
    );
...
async function executeCloudflareSandbox(options, targetDir) {
  ...
  try {
    ...
    try {
      ...
      if (options.verbose) {
        console.log(chalk.gray(`Using package manager: ${pm.name} v${pm.version}`));
      }
      ...
      await new Promise((resolve, reject) => {
        installProcess.on('close', async (installCode) => {
          if (installCode === 0) {
            ...
            console.log(
              chalk.red(`❌ ${pm.displayName} install failed with exit code ${installCode}`)
            );
            if (installError) {
  ...
  try {
    ...
    try {
      ...
      if (await fs.pathExists(componentsDir)) {
        ...
        await fs.writeFile(path.join(sandboxDir, 'e2b-launcher.py'), launcherContent, {
          mode: 0o755,
        });
    ...
    try {
      ...
      const pipInstall = spawn(
        pythonCmd,
        ['-m', 'pip', 'install', '-r', path.join(sandboxDir, 'requirements.txt')],
        {
          cwd: sandboxDir,
          stdio: 'pipe',
        }
          ...
          await new Promise((resolve, reject) => {
            ...
            sandboxExecution.on('close', (sandboxCode) => {
              ...
              if (sandboxCode === 0) {
                ...
                console.log(
                  chalk.yellow(`⚠️  Sandbox execution finished with exit code ${sandboxCode}`)
                );
            ...
            sandboxExecution.on('error', (error) => {
              ...
              if (error.code === 'TIMEOUT') {
                ...
                console.log(
                  chalk.gray('💡 This may happen with very complex prompts or large projects')
                );
                console.log(
                  chalk.blue(
