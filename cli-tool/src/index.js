...
const { runHooksDashboard } = require("./hooks-dashboard");
const { runUnifiedDashboard } = require("./unified-dashboard");
...
const {
  ...
  removeGlobalAgent,
  updateGlobalAgent,
} = require("./sdk/global-agent-manager");
...
async function showMainMenu() {
  ...
  if (initialChoice.action === "health") {
    ...
    if (healthResult.runSetup) {
      ...
      console.log(
        chalk.green("👍 Health check completed. Returning to main menu..."),
      );
...
async function createClaudeConfig(options = {}) {
  ...
  if (
    ...
    console.log(
      chalk.red(
        "❌ Error: --tunnel can only be used with --analytics, --chats, or --chats-mobile",
      ),
    );
  ...
  // Handle unified dashboard (all dashboards in one interface)
  if (options.dashboard) {
    trackingService.trackCommandExecution("unified-dashboard", {
      defaultView: options.defaultView || "analytics",
    });
    await runUnifiedDashboard(options);
    return;
  }

  // Handle analytics dashboard
  if (options.analytics) {
    trackingService.trackCommandExecution("analytics", {
  ...
  // Handle hooks dashboard
  if (options.hooks) {
    trackingService.trackCommandExecution("hooks-dashboard", {
      scope: options.scope || "all",
      host: options.host || "localhost",
      port: options.port || 3338,
    });
    trackingService.trackAnalyticsDashboard({
      page: "hooks-dashboard",
      source: "command_line",
    });
    await runHooksDashboard(options);
    return;
  }

  // Handle chats dashboard (now points to mobile chats interface)
  if (options.chats) {
    trackingService.trackCommandExecution("chats", {
      tunnel: options.tunnel || false,
    });
    trackingService.trackAnalyticsDashboard({
      page: "chats-mobile",
  ...
  if (options.agents) {
    trackingService.trackCommandExecution("agents", {
      tunnel: options.tunnel || false,
    });
    trackingService.trackAnalyticsDashboard({
      page: "agents",
  ...
  if (options.chatsMobile) {
    trackingService.trackCommandExecution("chats-mobile", {
      tunnel: options.tunnel || false,
    });
    trackingService.trackAnalyticsDashboard({
      page: "chats-mobile",
  ...
  if (options.yes) {
    ...
    const selectedLanguage =
      options.template ||
      options.language ||
      projectInfo.detectedLanguage ||
      "common";

    // Check if selected language is coming soon
    if (
      selectedLanguage &&
      TEMPLATES_CONFIG[selectedLanguage] &&
      TEMPLATES_CONFIG[selectedLanguage].comingSoon
    ) {
      console.log(
        chalk.red(`❌ ${selectedLanguage} is not available yet. Coming soon!`),
      );
      console.log(
        chalk.yellow(
          "Available languages: common, javascript-typescript, python",
        ),
      );
      return;
    }
    const availableHooks = getHooksForLanguage(selectedLanguage);
    const defaultHooks = availableHooks
      .filter((hook) => hook.checked)
      .map((hook) => hook.id);
    const availableMCPs = getMCPsForLanguage(selectedLanguage);
    const defaultMCPs = availableMCPs
      .filter((mcp) => mcp.checked)
      .map((mcp) => mcp.id);
  ...
  console.log("");
  console.log(
    chalk.blue("🌐 View all available templates at: https://aitmpl.com/"),
  );
  console.log(
    chalk.blue(
  ...
  if (config.language !== "common") {
    console.log(
      chalk.yellow(
        `💡 Language-specific features for ${config.language} have been configured`,
      ),
    );
  }
  ...
  if (config.framework !== "none") {
    console.log(
      chalk.yellow(
        `🎯 Framework-specific commands for ${config.framework} are available`,
      ),
    );
  }
  ...
  if (config.hooks && config.hooks.length > 0) {
    console.log(
      chalk.magenta(
        `🔧 ${config.hooks.length} automation hooks have been configured`,
      ),
    );
  }
  ...
  if (config.mcps && config.mcps.length > 0) {
    console.log(
      chalk.blue(`🔧 ${config.mcps.length} MCP servers have been configured`),
    );
  }

  // Track successful template installation
  if (!options.agent && !options.command && !options.mcp) {
    trackingService.trackTemplateInstallation(
      config.language,
      config.framework,
      {
        installation_method: options.setupFromMenu
          ? "interactive_menu"
          : "command_line",
...
async function installIndividualAgent(agentName, targetDir, options) {
  ...
  // Resolve scope for installation
  let scopeInfo = {
    targetDir: targetDir,
    claudeSubdir: ".claude",
    description: "project",
  ...
  if (options.scope) {
    scopeInfo = resolveScopeDirectory(options.scope, targetDir, "agents");
    if (!options.silent) {
      console.log(chalk.gray(`📍 Installing to ${scopeInfo.description}`));
    }
  }
  ...
  try {
    ...
    // Create .claude/agents directory if it doesn't exist (scope-aware)
    const agentsDir = path.join(
      scopeInfo.targetDir,
      scopeInfo.claudeSubdir,
      "agents",
    );
    ...
    if (!options.silent) {
      console.log(
        chalk.green(`✅ Agent "${agentName}" installed successfully!`),
      );
      console.log(
        chalk.cyan(
          `📁 Installed to: ${path.relative(scopeInfo.targetDir, targetFile)}`,
        ),
      );
    ...
    trackingService.trackDownload("agent", agentName, {
      ...
      target_directory: path.relative(process.cwd(), scopeInfo.targetDir),
      source: "github_main",
    });
...
async function installIndividualCommand(commandName, targetDir, options) {
  ...
  if (options.scope) {
    scopeInfo = resolveScopeDirectory(options.scope, targetDir, "commands");
    if (!options.silent) {
      console.log(chalk.gray(`📍 Installing to ${scopeInfo.description}`));
    }
  }
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        console.log(
          chalk.yellow("Available commands: check-file, generate-tests"),
        );
        return;
      }
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    ...
    // Create .claude/commands directory if it doesn't exist (scope-aware)
    ...
    if (!options.silent) {
      console.log(
        chalk.green(`✅ Command "${commandName}" installed successfully!`),
      );
      console.log(
        chalk.cyan(
          `📁 Installed to: ${path.relative(scopeInfo.targetDir, targetFile)}`,
...
async function installIndividualMCP(mcpName, targetDir, options) {
  ...
  if (options.scope) {
    scopeInfo = resolveScopeDirectory(options.scope, targetDir, "mcps");
    if (!options.silent) {
      console.log(chalk.gray(`📍 Installing to ${scopeInfo.description}`));
    }
  }
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        console.log(
          chalk.yellow(
            "Available MCPs: web-fetch, filesystem-access, github-integration, memory-integration, mysql-integration, postgresql-integration, deepgraph-react, deepgraph-nextjs, deepgraph-typescript, deepgraph-vue",
          ),
        );
        return;
      }
    ...
    // Check if .mcp.json exists in target directory (scope-aware)
    const targetMcpFile = path.join(scopeInfo.targetDir, ".mcp.json");
    let existingConfig = {};

    if (await fs.pathExists(targetMcpFile)) {
      existingConfig = await fs.readJson(targetMcpFile);
      console.log(
        chalk.yellow("📝 Existing .mcp.json found, merging configurations..."),
      );
    }
    ...
    if (!options.silent) {
      ...
      console.log(
        chalk.cyan(
          `📁 Configuration merged into: ${path.relative(
            scopeInfo.targetDir,
            targetMcpFile,
          )}`,
        ),
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        console.log(
          chalk.yellow(
            "Available settings: enable-telemetry, disable-telemetry, allow-npm-commands, deny-sensitive-files, use-sonnet, use-haiku, retention-7-days, retention-90-days",
          ),
        );
        console.log(
          chalk.yellow("Available statuslines: statusline/context-monitor"),
        );
        return;
      }
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    ...
    if (settingName.includes("statusline/")) {
      ...
      try {
        console.log(
          chalk.gray(`📥 Downloading Python script: ${pythonFileName}...`),
        );
        ...
        console.log(
          chalk.yellow(
            `⚠️  Could not download Python script: ${error.message}`,
          ),
        );
      }
    }
      ...
      const { selectedLocations } = await inquirer.prompt([
        {
          ...
          message:
            "Where would you like to install this setting? (Select one or more)",
          choices: [
            {
    ...
    for (const installLocation of installLocations) {
      ...
      if (installLocation === "user") {
        ...
        if (platform === "darwin") {
          ...
          console.log(
            chalk.yellow(
              "⚠️  Platform not supported for enterprise settings. Using user settings instead.",
            ),
          );
          const os = require("os");
          currentTargetDir = os.homedir();
        ...
        console.log(
          chalk.gray(
            `📍 Target path: ${path.join(currentTargetDir, settingsFile)}`,
          ),
        );
      }
      ...
      if (settingsFile === "managed-settings.json") {
        ...
        try {
          ...
          console.log(
            chalk.red(
              `❌ Failed to create enterprise directory: ${error.message}`,
            ),
          );
          console.log(
            chalk.yellow(
      ...
      if (await fs.pathExists(actualTargetFile)) {
        ...
        console.log(
          chalk.yellow(
            `📝 Existing ${settingsFile} found, merging configurations...`,
          ),
        );
      }
      ...
      if (existingConfig.env && settingConfig.env) {
        Object.keys(settingConfig.env).forEach((key) => {
          if (
            ...
            existingConfig.env[key] !== settingConfig.env[key]
          ) {
            conflicts.push(
              `Environment variable "${key}" (current: "${existingConfig.env[key]}", new: "${settingConfig.env[key]}")`,
            );
          }
        });
      }
      ...
      Object.keys(settingConfig).forEach((key) => {
        if (
          ...
          key !== "env" &&
          key !== "hooks" &&
          existingConfig[key] !== undefined &&
          JSON.stringify(existingConfig[key]) !==
            JSON.stringify(settingConfig[key])
        ) {
          ...
          if (
            ...
            typeof settingConfig[key] === "object" &&
            settingConfig[key] !== null
          ) {
            conflicts.push(
              `Setting "${key}" (will be overwritten with new configuration)`,
            );
          } else {
            conflicts.push(
              `Setting "${key}" (current: "${existingConfig[key]}", new: "${settingConfig[key]}")`,
            );
          }
        }
      });
      ...
      if (conflicts.length > 0) {
        ...
        if (!shouldOverwrite) {
          console.log(
            chalk.yellow(
              `⏹️  Installation of setting "${settingName}" in ${installLocation} cancelled by user.`,
            ),
          );
          continue; // Skip this location and continue with others
        }
      ...
      if (existingConfig.permissions && settingConfig.permissions) {
        ...
        ["allow", "deny", "ask"].forEach((key) => {
          if (
            existingConfig.permissions[key] &&
            settingConfig.permissions[key]
          ) {
      ...
      if (Object.keys(additionalFiles).length > 0) {
        console.log(
          chalk.blue(
            `📄 Installing ${
              Object.keys(additionalFiles).length
            } additional file(s)...`,
          ),
        );
        ...
        for (const [filePath, fileConfig] of Object.entries(additionalFiles)) {
          try {
            ...
            if (fileConfig.executable) {
              ...
              console.log(
                chalk.gray(`🔧 Made executable: ${resolvedFilePath}`),
              );
            }
            ...
            console.log(
              chalk.red(
                `❌ Failed to install file ${filePath}: ${fileError.message}`,
              ),
            );
          }
        }
      ...
      if (!options.silent) {
        ...
        console.log(
          chalk.cyan(`📁 Configuration merged into: ${actualTargetFile}`),
        );
        console.log(chalk.cyan(`📦 Downloaded from: ${githubUrl}`));
      }
    ...
    if (!options.silent) {
      if (successfulInstallations === installLocations.length) {
        ...
        console.log(
          chalk.red(
            `❌ ${failedCount} installation(s) failed due to permission or other errors.`,
          ),
        );
      }
    }
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404) {
        ...
        console.log(
          chalk.yellow(
            "Available hooks: notify-before-bash, format-python-files, format-javascript-files, git-add-changes, backup-before-edit, run-tests-after-changes",
          ),
        );
        return;
      }
    ...
    const pythonUrl = githubUrl.replace(".json", ".py");
    const hookBaseName = hookName.includes("/")
      ? hookName.split("/").pop()
      : hookName;
      ...
      const { selectedLocations } = await inquirer.prompt([
        {
          ...
          name: "selectedLocations",
          message:
            "Where would you like to install this hook? (Select one or more)",
          choices: [
            {
              name: "🏠 User settings (~/.claude/settings.json) - Applies to all projects",
              value: "user",
    ...
    for (const installLocation of installLocations) {
      ...
      if (await fs.pathExists(actualTargetFile)) {
        ...
        console.log(
          chalk.yellow(
            `📝 Existing ${settingsFile} found, merging hook configurations...`,
          ),
        );
      }
      ...
      if (conflicts.length > 0) {
        ...
        if (!shouldOverwrite) {
          console.log(
            chalk.yellow(
              `⏹️  Installation of hook "${hookName}" in ${installLocation} cancelled by user.`,
            ),
          );
          continue; // Skip this location and continue with others
        }
        ...
        Object.keys(hookConfig.hooks).forEach((hookType) => {
          if (!mergedConfig.hooks[hookType]) {
            ...
            if (Array.isArray(hookConfig.hooks[hookType])) {
              ...
              mergedConfig.hooks[hookType] = mergedConfig.hooks[
                hookType
              ].concat(hookConfig.hooks[hookType]);
            } else {
              // Old format compatibility: convert to new format
              console.log(
                chalk.yellow(
                  `⚠️  Converting old hook format to new Claude Code format for ${hookType}`,
                ),
              );
              if (!Array.isArray(mergedConfig.hooks[hookType])) {
                mergedConfig.hooks[hookType] = [];
      ...
      if (Object.keys(additionalFiles).length > 0) {
        for (const [relativePath, fileData] of Object.entries(
          ...
          // Write file
          await fs.writeFile(absolutePath, fileData.content, {
            mode: fileData.executable ? 0o755 : 0o644,
          });

          if (!options.silent) {
            console.log(
              chalk.green(`✓ Installed additional file: ${relativePath}`),
            );
          }
        }
      }
...
function resolveScopeDirectory(scope, targetDir, componentType) {
  const os = require("os");

  // Map 'global' to 'user' (alias)
  const normalizedScope = scope === "global" ? "user" : scope;

  const scopeMap = {
    user: {
      targetDir: os.homedir(),
      claudeSubdir: ".claude",
      description: "~/.claude (user-level, all projects)",
      settingsFile: "settings.json",
    },
    project: {
      targetDir: targetDir,
      claudeSubdir: ".claude",
      description: "./.claude (project-level, shared)",
      settingsFile: "settings.json",
    },
    local: {
      targetDir: targetDir,
      claudeSubdir: ".claude",
      description: "./.claude (project-level, not committed)",
      settingsFile: "settings.local.json",
    },
  };

  // Enterprise only for settings/hooks
  if (
    normalizedScope === "enterprise" &&
    (componentType === "settings" || componentType === "hooks")
  ) {
    return getEnterpriseDirectory();
  }

  if (!scopeMap[normalizedScope]) {
    throw new Error(
      `Invalid scope: ${scope}. Valid: user, global, project, local`,
    );
  }
...
function getEnterpriseDirectory() {
  const os = require("os");
  const platform = os.platform();
  ...
  return {
    targetDir: enterpriseMap[platform] || enterpriseMap.linux,
    claudeSubdir: "",
    description: "Enterprise (system-wide, requires admin)",
    settingsFile: "managed-settings.json",
  };
}
...
function extractLanguageFromAgent(content, agentName) {
  ...
  if (agentName.includes("react") || content.includes("React"))
    return "javascript-typescript";
  if (agentName.includes("django") || content.includes("Django"))
    return "python";
  if (agentName.includes("fastapi") || content.includes("FastAPI"))
    return "python";
  ...
  if (agentName.includes("api-security") || content.includes("API security"))
    return "javascript-typescript";
  if (agentName.includes("database") || content.includes("database"))
    return "javascript-typescript";
  ...
  try {
    ...
    if (fs.existsSync(componentsPath)) {
      ...
      if (componentsData.agents && Array.isArray(componentsData.agents)) {
        ...
        console.log(
          chalk.green(`✅ Loaded ${agents.length} agents from local cache`),
        );
        return agents;
      }
    }
    ...
    try {
      ...
      if (apiResponse.ok) {
        ...
        if (apiData.agents && Array.isArray(apiData.agents)) {
          console.log(
            chalk.green(
              `✅ Loaded ${apiData.agents.length} agents from aitmpl.com API`,
            ),
          );
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        if (responseText.includes("rate limit")) {
          ...
          console.log(
            chalk.yellow(
              "💡 Install locally with: npm install -g claude-code-templates",
            ),
          );
          ...
          return [
            ...
            },
            {
              ...
              path: "development-team/backend-developer",
              category: "development-team",
            },
            {
            ...
            },
            {
              ...
              path: "web-tools/react-developer",
              category: "web-tools",
            },
            {
              name: "vue-developer",
              path: "web-tools/vue-developer",
              category: "web-tools",
            },
            {
              name: "data-scientist",
              path: "data-analytics/data-scientist",
              category: "data-analytics",
            },
            {
              name: "data-analyst",
              path: "data-analytics/data-analyst",
              category: "data-analytics",
            },
            {
...
async function installIndividualSkill(skillName, targetDir, options) {
  ...
  if (options.scope) {
    scopeInfo = resolveScopeDirectory(options.scope, targetDir, "skills");
    if (!options.silent) {
      console.log(chalk.gray(`📍 Installing to ${scopeInfo.description}`));
    }
  }
  ...
  try {
    ...
    const skillBaseName = skillName.includes("/")
      ? skillName.split("/").pop()
      : skillName;
    ...
    console.log(
      chalk.gray(`📥 Downloading skill from GitHub (main branch)...`),
    );
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            ...
            console.log(
              chalk.yellow(
                '💡 Tip: Use format "category/skill-name" (e.g., creative-design/algorithmic-art)',
              ),
            );
            console.log(
              chalk.yellow(
                "Available categories: creative-design, development, document-processing, enterprise-communication",
              ),
            );
            return false;
          }
          ...
          if (item.type === "file") {
            ...
            try {
              ...
              if (fileResponse.ok) {
                ...
                const isExecutable =
                  item.name.endsWith(".py") || item.name.endsWith(".sh");
        ...
        console.log(
          chalk.gray(`  (Could not access GitHub API: ${error.message})`),
        );
        return false;
      }
    }
    ...
    // Create .claude/skills/skill-name directory (Anthropic standard structure, scope-aware)
    ...
    for (const [filePath, fileData] of Object.entries(downloadedFiles)) {
      const fullPath = path.join(scopeInfo.targetDir, filePath);
    ...
    if (!options.silent) {
      ...
      console.log(
        chalk.cyan(
          `📄 Total files downloaded: ${Object.keys(downloadedFiles).length}`,
        ),
      );
...
async function installMultipleComponents(options, targetDir) {
  ...
  try {
    ...
    if (options.agent) {
      const agentsInput = Array.isArray(options.agent)
        ? options.agent.join(",")
        : options.agent;
      components.agents = agentsInput
        .split(",")
        .map((a) => a.trim())
        .filter((a) => a);
    }
    ...
    if (options.command) {
      const commandsInput = Array.isArray(options.command)
        ? options.command.join(",")
        : options.command;
      components.commands = commandsInput
        .split(",")
        .map((c) => c.trim())
        .filter((c) => c);
    }
    ...
    if (options.mcp) {
      const mcpsInput = Array.isArray(options.mcp)
        ? options.mcp.join(",")
        : options.mcp;
      components.mcps = mcpsInput
        .split(",")
        .map((m) => m.trim())
        .filter((m) => m);
    }
    ...
    if (options.setting) {
      const settingsInput = Array.isArray(options.setting)
        ? options.setting.join(",")
        : options.setting;
    ...
    if (options.hook) {
      const hooksInput = Array.isArray(options.hook)
        ? options.hook.join(",")
        : options.hook;
      components.hooks = hooksInput
        .split(",")
        .map((h) => h.trim())
        .filter((h) => h);
    }
    ...
    if (options.skill) {
      const skillsInput = Array.isArray(options.skill)
        ? options.skill.join(",")
        : options.skill;
      components.skills = skillsInput
        .split(",")
        .map((s) => s.trim())
        .filter((s) => s);
    }
    ...
    // Parse and validate --scope flag
    let scopeLocations = null;
    if (options.scope) {
      const validScopes = ["user", "global", "project", "local", "enterprise"];
      const normalizedScopeInput = options.scope.toLowerCase().trim();

      if (!validScopes.includes(normalizedScopeInput)) {
        throw new Error(
          `Invalid scope: ${options.scope}. Valid scopes: user, global, project, local, enterprise`,
        );
      }

      // Map 'global' to 'user' (alias)
      const normalizedScope =
        normalizedScopeInput === "global" ? "user" : normalizedScopeInput;
      scopeLocations = [normalizedScope];

      if (normalizedScope === "enterprise") {
        console.log(
          chalk.yellow(
            "⚠️  Enterprise scope requires administrator privileges",
          ),
        );
      }

      console.log(
        chalk.cyan(`📍 Installing all components to scope: ${normalizedScope}`),
      );
    }

    // Ask for installation locations once for configuration components (if any exist and not in silent mode)
    let sharedInstallLocations = scopeLocations || ["local"]; // use scope or default
    const hasSettingsOrHooks =
      components.settings.length > 0 || components.hooks.length > 0;

    if (hasSettingsOrHooks && !options.yes && !scopeLocations) {
      console.log(
        chalk.blue(
      ...
      const { selectedLocations } = await inquirer.prompt([
        {
          ...
          name: "selectedLocations",
          message:
            "Where would you like to install the configuration components? (Select one or more)",
          choices: [
            {
              name: "🏠 User settings (~/.claude/settings.json) - Applies to all projects",
              value: "user",
      ...
      console.log(
        chalk.cyan(
          `📋 Will install configuration components in: ${sharedInstallLocations.join(
            ", ",
          )}`,
        ),
      );
    } else if (scopeLocations && hasSettingsOrHooks) {
      console.log(
        chalk.cyan(
          `📋 Using --scope for configuration components: ${scopeLocations.join(", ")}`,
        ),
      );
    }
    ...
    for (const agent of components.agents) {
      ...
      const agentSuccess = await installIndividualAgent(agent, targetDir, {
        ...options,
        silent: true,
        scope: scopeLocations ? scopeLocations[0] : undefined,
      });
    ...
    for (const command of components.commands) {
      ...
      const commandSuccess = await installIndividualCommand(
        command,
        targetDir,
        {
          ...options,
    ...
    for (const mcp of components.mcps) {
      ...
      const mcpSuccess = await installIndividualMCP(mcp, targetDir, {
        ...options,
        silent: true,
        scope: scopeLocations ? scopeLocations[0] : undefined,
      });
    ...
    for (const setting of components.settings) {
      ...
      const settingSuccess = await installIndividualSetting(
        setting,
        targetDir,
        {
          ...options,
    ...
    for (const skill of components.skills) {
      ...
      const skillSuccess = await installIndividualSkill(skill, targetDir, {
        ...options,
        silent: true,
        scope: scopeLocations ? scopeLocations[0] : undefined,
      });
    ...
    if (options.yaml) {
      ...
      try {
        ...
        const yamlContent = Buffer.from(options.yaml, "base64").toString(
          "utf8",
        );
        ...
        console.log(
          chalk.red(`❌ Error processing YAML: ${yamlError.message}`),
        );
      }
    }
    ...
    if (successfullyInstalled === totalComponents) {
      ...
      console.log(
        chalk.red(
          `❌ ${
            totalComponents - successfullyInstalled
          } component(s) failed to install.`,
        ),
      );
    } else {
    ...
    if (options.yaml) {
      console.log(
        chalk.cyan(`📄 Workflow file created in: .claude/workflows/`),
      );
      console.log(
        chalk.cyan(
          `🚀 Use the workflow file with Claude Code to execute the complete setup`,
        ),
      );
    }
...
async function showAvailableAgents() {
  ...
  console.log(
    chalk.gray(
      "Use format: category/agent-name or just agent-name for root level\n",
    ),
  );
  ...
  const groupedAgents = agents.reduce((acc, agent) => {
    const category =
      agent.category === "root" ? "🤖 General Agents" : `📁 ${agent.category}`;
  ...
  console.log(
    chalk.gray("  cct --agent deep-research-team/academic-researcher"),
  );
  console.log("");
}
...
async function installWorkflow(workflowHash, targetDir, options) {
  ...
  try {
    ...
    const hash = workflowHash.startsWith("#")
      ? workflowHash.substring(1)
      : workflowHash;
    ...
    const commands = workflowData.steps.filter(
    ...
    if (workflowData.components) {
      console.log(
        chalk.blue(`📦 Installing components from workflow package...`),
      );
      ...
      if (workflowData.components.agent) {
        for (const agent of workflowData.components.agent) {
          ...
          await installComponentFromWorkflow(
            agent,
            "agent",
            targetDir,
            options,
      ...
      console.log(
        chalk.yellow(`⚠️  Using legacy component installation method...`),
      );

      // Install agents
      for (const agent of agents) {
        console.log(chalk.gray(`   Installing agent: ${agent.name}`));
        await installIndividualAgent(agent.path, targetDir, {
          ...options,
          silent: true,
        });
      }
      ...
      for (const command of commands) {
        console.log(chalk.gray(`   Installing command: ${command.name}`));
        await installIndividualCommand(command.path, targetDir, {
          ...options,
          silent: true,
        });
      }
    ...
    for (const mcp of mcps) {
      console.log(chalk.gray(`   Installing MCP: ${mcp.name}`));
      await installIndividualMCP(mcp.path, targetDir, {
        ...options,
        silent: true,
      });
    }
    ...
    const workflowFile = path.join(
      workflowsDir,
      `${workflowData.name.replace(/[^a-z0-9]/gi, "_").toLowerCase()}.yaml`,
    );
    ...
    console.log(chalk.cyan(`📁 Components installed to: .claude/`));
    console.log(
      chalk.cyan(`📄 Workflow file: ${path.relative(targetDir, workflowFile)}`),
    );
    console.log(
      chalk.cyan(
        `🚀 Use the workflow file with Claude Code to execute the complete workflow`,
      ),
    );
    ...
    if (error.message.includes("not found")) {
      ...
      console.log(
        chalk.gray("   • Verify the workflow was generated successfully"),
      );
      console.log(
        chalk.gray("   • Try generating a new workflow from the builder"),
      );
    }
  }
}
...
async function fetchWorkflowData(hash) {
  try {
    ...
    if (hash === "demo123" || hash === "abc123test") {
      console.log(
        chalk.green("🎯 Demo workflow found! Using sample configuration..."),
      );
      return {
        name: "Full Stack Development Workflow",
        description:
          "Complete workflow for setting up a full-stack development environment with React frontend, Node.js backend, and security auditing",
    ...
    console.error(
      chalk.red(`❌ Error fetching workflow data: ${error.message}`),
    );
    return null;
  }
}
 ...
 */
async function installComponentFromWorkflow(
  componentData,
  type,
  targetDir,
  options,
  ...
  try {
    ...
    console.error(
      chalk.red(
        `❌ Error installing ${type} "${componentData.name}": ${error.message}`,
      ),
    );
    throw error;
  }
...
${workflowData.steps
  .map(
    ...
    claude_instructions: |
      Execute this step using the ${step.type} located at .claude/${
        step.type
      }s/${step.name}.${step.type === "mcp" ? "json" : "md"}
      Task: ${step.description}
      ${
        step.type === "agent"
          ? "Use this agent to perform the specified task with full context from previous steps."
          : ""
      }
      ${
        step.type === "command"
          ? "Execute this command with appropriate parameters based on workflow context."
          : ""
      }
      ${
        step.type === "mcp"
          ? "Ensure MCP server is running and utilize its capabilities for the task."
          : ""
      }
...
components:
  agents: [${workflowData.steps
    .filter((s) => s.type === "agent")
    .map((s) => `"${s.path}"`)
    .join(", ")}]
  commands: [${workflowData.steps
    .filter((s) => s.type === "command")
    .map((s) => `"${s.path}"`)
    .join(", ")}]
  mcps: [${workflowData.steps
    .filter((s) => s.type === "mcp")
    .map((s) => `"${s.path}"`)
    .join(", ")}]
...
${workflowData.steps
  .map(
    (step, index) =>
      `# ${index + 1}. ${step.description} (${step.type}: ${step.name})`,
  )
  .join("\n")}
#
# Total estimated time: ${Math.ceil(workflowData.steps.length * 2)} minutes
# Components required: ${
    workflowData.steps.filter((s) => s.type === "agent").length
  } agents, ${
    workflowData.steps.filter((s) => s.type === "command").length
  } commands, ${workflowData.steps.filter((s) => s.type === "mcp").length} MCPs
#═══════════════════════════════════════════════════════════════════════════════
`;
...
async function handlePromptExecution(prompt, targetDir) {
  ...
  console.log(
    chalk.blue("🚀 Preparing to launch Claude Code with your prompt..."),
  );
  ...
  try {
    ...
    claudeProcess.on("error", async (error) => {
      if (error.code === "ENOENT") {
        ...
        console.log(
          chalk.gray("   1. Install Claude Code CLI: https://claude.ai/code"),
        );
        console.log(
          chalk.gray(
            "   2. Copy and paste this prompt in Claude Code interface:",
          ),
        );
        ...
        if (openWeb) {
          ...
          console.log(
            chalk.cyan(`Don't forget to paste your prompt: "${prompt}"`),
          );
        }
      } else {
        throw error;
    ...
    console.log(
      chalk.blue("💡 You can manually execute this prompt in Claude Code:"),
    );
...
async function launchClaudeCodeStudio(options, targetDir) {
  ...
  console.log(
    chalk.gray("💡 This interface supports both local and cloud execution"),
  );
  ...
  setTimeout(async () => {
    try {
      ...
      console.log(
        chalk.green("✅ Claude Code Studio launched at http://localhost:3444"),
      );
      console.log(
        chalk.gray("💡 Choose between Local Machine or E2B Cloud execution"),
      );
    } catch (error) {
      console.log(
        chalk.yellow("💡 Please manually open: http://localhost:3444"),
      );
    }
  }, 2000);
...
async function executeSandbox(options, targetDir) {
  ...
  if (sandbox !== "e2b" && sandbox !== "cloudflare" && sandbox !== "docker") {
    ...
    console.log(
      chalk.yellow("💡 Available providers: e2b, cloudflare, docker"),
    );
    console.log(
      chalk.gray('   Example: --sandbox e2b --prompt "Create a web app"'),
    );
    console.log(
      chalk.gray(
        '   Example: --sandbox cloudflare --prompt "Calculate factorial of 5"',
      ),
    );
    console.log(
      chalk.gray('   Example: --sandbox docker --prompt "Write a function"'),
    );
    return;
  }
  ...
  if (!agent && !yes) {
    ...
    console.log(
      chalk.gray(
        "Select one or more agents for your task (use SPACE to select, ENTER to confirm).\n",
      ),
    );
    ...
    if (wantAgents) {
      ...
      if (selectedAgents && selectedAgents.length > 0) {
        ...
        console.log(
          chalk.green(
            `✅ Selected agents: ${chalk.cyan(selectedAgents.join(", "))}`,
          ),
        );
      } else {
    ...
    console.log(
      chalk.yellow("⚠️ No agent specified, continuing without specific agents"),
    );
  }
  ...
  if (!prompt) {
    ...
    console.log(
      chalk.gray(
        "Describe what you want to create in detail. The more specific you are,",
      ),
    );
    console.log(
      chalk.gray("the better Claude Code will understand your requirements.\n"),
    );
  ...
  if (sandbox === "e2b") {
    ...
    if (!e2bKey) {
      ...
      console.log(
        chalk.gray("   1. Set environment variable: E2B_API_KEY=your_key"),
      );
      console.log(
        chalk.gray("   2. Use CLI parameter: --e2b-api-key your_key"),
      );
    ...
    if (!anthropicKey) {
      ...
      console.log(
        chalk.gray(
          "   1. Set environment variable: ANTHROPIC_API_KEY=your_key",
        ),
      );
      console.log(
        chalk.gray("   2. Use CLI parameter: --anthropic-api-key your_key"),
      );
      console.log(
        chalk.blue("   Get your key at: https://console.anthropic.com"),
      );
      return;
    }

    // Execute E2B sandbox
    await executeE2BSandbox(
      {
        sandbox,
        agent,
        prompt,
    ...
    if (!anthropicKey) {
      console.log(
        chalk.red(
          "❌ Error: Anthropic API key is required for Cloudflare sandbox",
        ),
      );
      console.log(chalk.yellow("💡 Options:"));
      console.log(
    ...
    await executeCloudflareSandbox(
      { sandbox, agent, prompt, command, mcp, setting, hook, anthropicKey },
      targetDir,
    );
  } else if (sandbox === "docker") {
    if (!anthropicKey) {
      console.log(
        chalk.red("❌ Error: Anthropic API key is required for Docker sandbox"),
      );
      console.log(chalk.yellow("💡 Options:"));
      console.log(
        chalk.gray(
...
async function executeCloudflareSandbox(options, targetDir) {
  ...
  console.log(
    chalk.white(`💭 Prompt: ${chalk.cyan('"' + truncatedPrompt + '"')}`),
  );
  ...
  if (!shouldExecute) {
    console.log(
      chalk.yellow("⏹️  Cloudflare sandbox execution cancelled by user."),
    );
    return;
  }
  ...
  try {
    ...
    try {
      ...
      if (!nodeAvailable) {
        ...
        console.log(
          chalk.red("❌ Node.js 16.17.0+ is required for Cloudflare sandbox"),
        );
      ...
      // Detect package manager (Bun or npm)
      const pm = await detectPackageManager({ verbose: options.verbose });
      if (options.verbose) {
        console.log(
          chalk.gray(`Using package manager: ${pm.name} v${pm.version}`),
        );
      }

      // Install dependencies
      const depSpinner = ora(
        `Installing Cloudflare dependencies with ${pm.displayName}...`,
      ).start();

      const installProcess = spawn(pm.installCmd, ["install"], {
        cwd: sandboxDir,
        stdio: "pipe",
      ...
      await new Promise((resolve, reject) => {
        installProcess.on("close", async (installCode) => {
          if (installCode === 0) {
            depSpinner.succeed(
              "Cloudflare dependencies installed successfully",
            );
            ...
            if (componentsToInstall) {
              console.log(
                chalk.gray(`📦 Components to install:${componentsToInstall}`),
              );
            }
            ...
            console.log(
              chalk.blue("🚀 Starting Cloudflare sandbox execution..."),
            );

            const sandboxExecution = spawn(
              "npx",
              [
                "tsx",
                launcherPath,
            ...
            sandboxExecution.on("close", (code) => {
              if (code === 0) {
                console.log(
                  chalk.green(
                    "🎉 Cloudflare sandbox execution completed successfully!",
                  ),
                );
                resolve();
              } else if (code === null) {
                console.log(
                  chalk.yellow("⏹️  Sandbox execution was cancelled"),
                );
                resolve();
              } else {
                console.log(
                  chalk.yellow(
                    `⚠️  Sandbox execution finished with exit code ${code}`,
                  ),
                );
                console.log(
                  chalk.gray("💡 Check the output above for error details"),
                );
                resolve();
              }
            });
            ...
            sandboxExecution.on("error", (error) => {
              console.log(
                chalk.red(`❌ Error executing sandbox: ${error.message}`),
              );
              console.log(
                chalk.yellow("💡 Make sure you have set ANTHROPIC_API_KEY"),
              );
              reject(error);
            });
          } else {
            ...
            console.log(
              chalk.red(
                `❌ ${pm.displayName} install failed with exit code ${installCode}`,
              ),
            );
            if (installError) {
              console.log(chalk.red("Error output:"));
              console.log(chalk.gray(installError.trim()));
            }
    ...
    console.log(
      chalk.red(`❌ Error setting up Cloudflare sandbox: ${error.message}`),
    );
    console.log(
...
async function executeDockerSandbox(options, targetDir) {
  const { agent, command, mcp, setting, hook, prompt, anthropicKey, yes } =
    options;
  ...
  if (!yes) {
    ...
    if (!shouldExecute) {
      console.log(
        chalk.yellow("⏹️  Docker sandbox execution cancelled by user."),
      );
      return;
    }
  }
  ...
  try {
    ...
    try {
      if (await fs.pathExists(componentsDir)) {
        ...
        if (copiedFiles.length === 0) {
          throw new Error(
            "No files were copied from Docker component directory",
          );
        }
      } else {
        throw new Error(
    ...
    try {
      ...
      if (!dockerAvailable) {
        ...
        console.log(
          chalk.blue("   Visit: https://docs.docker.com/get-docker/"),
        );
        return;
      }
      ...
      const dockerExec = spawn(
        "node",
        [launcherPath, prompt, componentsToInstall.trim()],
        {
          cwd: sandboxDir,
          stdio: "inherit",
          env: {
      ...
      await new Promise((resolve, reject) => {
        dockerExec.on("close", (dockerCode) => {
          if (dockerCode === 0) {
            execSpinner.succeed(
              "Docker sandbox execution completed successfully",
            );
            console.log(chalk.green("\n✅ Docker sandbox execution finished!"));
            console.log(
              chalk.white("📁 Output files are in the output/ directory"),
            );
            resolve();
          } else {
            execSpinner.fail(
              `Docker sandbox execution failed with code ${dockerCode}`,
            );
            reject(
              new Error(`Docker execution failed with code ${dockerCode}`),
            );
          }
        });
    ...
    console.log(
      chalk.red(`❌ Error setting up Docker sandbox: ${error.message}`),
    );
    console.log(
      chalk.yellow("💡 Please check your Docker installation and try again"),
    );
  }
}

async function executeE2BSandbox(options, targetDir) {
  const { agent, prompt, command, mcp, setting, hook, e2bKey, anthropicKey } =
    options;
  ...
  console.log(
    chalk.gray(`   • Files will be downloaded to: ${chalk.cyan(targetDir)}`),
  );
  console.log(
    chalk.gray("   • Extended timeout: 15 minutes for complex operations"),
  );
  ...
  try {
    ...
    try {
      ...
      if (await fs.pathExists(componentsDir)) {
        ...
        if (await fs.pathExists(requirementsPath)) {
          await fs.copyFile(
            requirementsPath,
            path.join(sandboxDir, "requirements.txt"),
          );
        }
        ...
        console.log(
          chalk.gray("📥 Downloading E2B component files from GitHub..."),
        );

        const baseUrl =
          "https://raw.githubusercontent.com/davila7/claude-code-templates/main/cli-tool/components/sandbox/e2b";

        // Download launcher script
        const launcherResponse = await fetch(`${baseUrl}/e2b-launcher.py`);
        if (!launcherResponse.ok) {
          throw new Error(
            `Failed to download e2b-launcher.py: ${launcherResponse.status} ${launcherResponse.statusText}`,
          );
        }
        ...
        if (!requirementsResponse.ok) {
          throw new Error(
            `Failed to download requirements.txt: ${requirementsResponse.status} ${requirementsResponse.statusText}`,
          );
        }
        ...
        if (!envExampleResponse.ok) {
          throw new Error(
            `Failed to download .env.example: ${envExampleResponse.status} ${envExampleResponse.statusText}`,
          );
        }
    ...
    try {
      ...
      if (python311Available) {
        ...
        console.log(
          chalk.yellow("⚠ Python 3.11 not found, trying platform defaults..."),
        );
      ...
      if (!pythonCmd) {
        ...
        console.log(
          chalk.yellow("💡 Please install Python 3.11+ and try again"),
        );
      ...
      pipInstall.on("close", async (pipCode) => {
        if (pipCode === 0) {
          ...
          // Execute sandbox
          console.log(
            chalk.blue("🚀 Launching E2B sandbox with Claude Code..."),
          );
          console.log(
            chalk.gray(
              `📝 Prompt: "${prompt.substring(0, 100)}${
          ...
          console.log(
            chalk.cyan(
              "⏱️  Extended timeout: 15 minutes for complex operations",
            ),
          );
          ...
          console.log(
            chalk.yellow("💡 Press ESC anytime to cancel the execution"),
          );
          ...
          await new Promise((resolve, reject) => {
            ...
            sandboxExecution.on("close", (sandboxCode) => {
              ...
              if (sandboxCode === 0) {
                console.log(
                  chalk.green("🎉 Sandbox execution completed successfully!"),
                );
                console.log(
                  chalk.blue(
                    "💡 Files were created inside the E2B sandbox environment",
                  ),
                );
                resolve();
                ...
                console.log(
                  chalk.yellow(
                    `⚠️  Sandbox execution finished with exit code ${sandboxCode}`,
                  ),
                );
                console.log(
                  chalk.gray("💡 Check the output above for any error details"),
                );
            ...
            sandboxExecution.on("error", (error) => {
              ...
              if (error.code === "TIMEOUT") {
                console.log(
                  chalk.yellow(
                    "⏱️  Sandbox execution timed out after 15 minutes",
                  ),
                );
                console.log(
                  chalk.gray(
                    "💡 This may happen with very complex prompts or large projects",
                  ),
                );
                console.log(
                  chalk.blue(
                    "💡 Try breaking down your prompt into smaller, more specific requests",
                  ),
                );
              } else {
                console.log(
                ...
                console.log(
                  chalk.yellow(
                    "💡 Make sure you have set E2B_API_KEY and ANTHROPIC_API_KEY environment variables",
                  ),
                );
                console.log(
                  chalk.gray(
                    "   Create a .env file in the .claude/sandbox directory with your API keys",
                  ),
                );
              }
              reject(error);
          ...
          console.log(
            chalk.red(`❌ pip install failed with exit code ${pipCode}`),
          );
          if (pipError) {
            console.log(chalk.red("Error output:"));
          ...
          console.log(
            chalk.gray(`   ${pythonCmd} -m pip install -r requirements.txt`),
          );
