...
class StructuralValidator extends BaseValidator {
  ...
  validateEncoding(content, path) {
    try {
      ...
      if (content.includes('\0')) {
        this.addError('STRUCT_E005', 'File contains null bytes (possible binary content)', {
          path,
        });
      }
    } catch (error) {
  ...
  validateFrontmatter(content, path) {
    ...
    try {
      ...
      if (!frontmatter || typeof frontmatter !== 'object') {
        this.addError('STRUCT_E002', 'Frontmatter is empty or not a valid object', { path });
        return null;
      }
      ...
      this.addError('STRUCT_E002', `Invalid YAML syntax in frontmatter: ${error.message}`, {
        path,
        error: error.message,
      });
      return null;
  ...
  validateTools(frontmatter, path) {
    ...
    if (typeof tools === 'string') {
      ...
      if (invalidTools.length > 0) {
        this.addWarning('STRUCT_W006', `Unknown tools specified: ${invalidTools.join(', ')}`, {
          path,
          invalidTools,
        });
      }
  ...
  checkRecommendedFields(frontmatter, type, path) {
    ...
    if (missingFields.length > 0) {
      this.addInfo('STRUCT_I003', `Missing recommended fields: ${missingFields.join(', ')}`, {
        path,
        missingFields,
      });
    }
  ...
  validateContentStructure(content, path) {
    ...
    if (contentWithoutFrontmatter.trim().length < 50) {
      this.addWarning('STRUCT_W009', 'Component content is very short (less than 50 characters)', {
        path,
        length: contentWithoutFrontmatter.trim().length,
      });
    }
