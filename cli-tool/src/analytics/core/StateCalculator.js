...
class StateCalculator {
  ...
  determineConversationState(messages, lastModified, runningProcess = null) {
    ...
    const claudeActivity = this.detectRealClaudeActivity(
      messages,
      lastModified,
    );
  ...
  quickStateCalculation(conversation, runningProcesses) {
    ...
    const hasActiveProcess = runningProcesses.some(
      (process) =>
  ...
  detectRealClaudeActivity(messages, lastModified) {
    ...
    const hasRecentTools = recentMessages.some(
      (msg) =>
    ...
    if (sortedMessages.length >= 2) {
      ...
      const timeBetweenLast =
        Math.abs(
          new Date(lastTwoMessages[1].timestamp) -
            new Date(lastTwoMessages[0].timestamp),
        ) /
        (1000 * 60); // minutes

      if (
        timeBetweenLast < 5 &&
        messageMinutesAgo < 15 &&
        fileMinutesAgo < 20
      ) {
  ...
  getStateClass(conversationState) {
    if (
      conversationState.includes("working") ||
      conversationState.includes("Working")
    ) {
      return "working";
    }
    if (
      conversationState.includes("typing") ||
      conversationState.includes("Typing")
    ) {
      return "typing";
    }
