...
function categorizeServer(serverConfig) {
  ...
  if (
    ...
    description.includes("filesystem")
  ) {
    return "Filesystem";
  }
...
function calculateComplexity(serverConfig) {
  ...
  if (serverConfig.env && Object.keys(serverConfig.env).length > 0)
    complexity += 1;
  if (serverConfig.settings && Object.keys(serverConfig.settings).length > 0)
    complexity += 1;
...
function displayMCPStats(analysis) {
  ...
  console.log(
    chalk.gray(`Last Modified: ${analysis.lastModified.toLocaleDateString()}`),
  );
  ...
  analysis.servers.forEach((server) => {
    ...
    const truncatedCommand =
      server.command.length > 12
        ? server.command.substring(0, 9) + "..."
        : server.command;

    const complexityStars =
      "★".repeat(server.complexity) + "☆".repeat(5 - server.complexity);

    const truncatedDesc =
      server.description.length > 30
  ...
  for (const [category, servers] of Object.entries(analysis.byCategory)) {
    ...
    console.log(
      chalk.gray(`  ${category}: ${servers.length} servers`) +
        chalk.green(` (${enabledCount} enabled`) +
        (disabledCount > 0
          ? chalk.yellow(`, ${disabledCount} disabled)`)
          : chalk.green(")")),
    );
  }
  ...
  analysis.servers.forEach((server) => {
    complexityCount[server.complexity] =
      (complexityCount[server.complexity] || 0) + 1;
  });
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (count > 0) {
      ...
      console.log(
        chalk.gray(`  ${chalk.yellow(stars)} (${i}/5): ${count} servers`),
      );
    }
  }
}
  ...
  const { optimizeMCP } = await inquirer.prompt([
    {
      ...
      name: "optimizeMCP",
      message:
        "Would you like Claude Code to review and optimize your MCP server configuration?",
      default: true,
    },
  ]);
...
Consider my project structure and development needs to suggest the most beneficial MCP server setup.`;
    ...
    try {
      ...
      child.on("error", (error) => {
        console.error(
          chalk.red("❌ Error launching Claude Code:"),
          error.message,
        );
        console.log(
          chalk.yellow(
      ...
      console.log(
        chalk.yellow("💡 Make sure Claude Code is installed and accessible."),
      );
    }
  } else {
    console.log(
