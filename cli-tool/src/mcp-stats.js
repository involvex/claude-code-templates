...
function displayMCPStats(analysis) {
  ...
  analysis.servers.forEach((server) => {
    ...
    const truncatedCommand =
      server.command.length > 12 ? server.command.substring(0, 9) + '...' : server.command;
...
async function runMCPStats(options) {
  ...
  console.log(
    chalk.gray('Claude Code can analyze your MCP server configuration and suggest optimizations.')
  );
