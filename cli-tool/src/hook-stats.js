...
function estimateTokens(text) {
  ...
  const adjustedTokens =
    baseTokens + codeBlocks * 5 + inlineCode * 2 + jsonBlocks * 3;
...
async function analyzeHooks(targetDir = process.cwd()) {
  ...
  try {
    ...
    if (!settings.hooks || Object.keys(settings.hooks).length === 0) {
      console.log(
        chalk.yellow("⚠️  No automation hooks found in settings.json."),
      );
      return null;
    }
    ...
    for (const [hookType, hooks] of Object.entries(settings.hooks)) {
      ...
      for (const hook of hooks) {
        const hookData = {
          ...
          words: (hook.script || hook.command || "")
            .split(/\s+/)
...
function displayHookStats(analysis) {
  ...
  console.log(
    chalk.gray(`File: .claude/settings.json (${analysis.fileSize} bytes)`),
  );
  console.log(
  ...
  for (const [type, data] of Object.entries(analysis.byType)) {
    ...
    console.log(
      chalk.gray(`  ${type}: ${data.count} hooks`) +
  ...
  const { optimizeHooks } = await inquirer.prompt([
    {
      ...
      name: "optimizeHooks",
      message:
        "Would you like Claude Code to review and optimize your automation hooks?",
      default: true,
    },
  ]);
