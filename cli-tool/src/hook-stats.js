    ...
    for (const [hookType, hooks] of Object.entries(settings.hooks)) {
      ...
      for (const hook of hooks) {
        const hookData = {
          ...
          words: (hook.script || hook.command || '').split(/\s+/).filter((word) => word.length > 0)
            .length,
...
function displayHookStats(analysis) {
  ...
  const separator =
    '─'.repeat(nameWidth) + '─┼─' + '─'.repeat(12) + '─┼─' + '─'.repeat(8) + '─┼─' + '─'.repeat(40);
  ...
  analysis.hooks.forEach((hook) => {
    const truncatedName =
      hook.name.length > nameWidth ? hook.name.substring(0, nameWidth - 3) + '...' : hook.name;

    const statusColor = hook.enabled ? chalk.green : chalk.yellow;
    const status = hook.enabled ? 'Enabled' : 'Disabled';

    const truncatedDesc =
      hook.description.length > 40 ? hook.description.substring(0, 37) + '...' : hook.description;
...
async function runHookStats(options) {
  ...
  if (!analysis) {
    ...
    console.log(
      chalk.gray('Would you like to set up Claude Code Templates to add automation hooks?')
    );
  ...
  console.log(
    chalk.gray('Claude Code can analyze your automation hooks and suggest optimizations.')
  );
