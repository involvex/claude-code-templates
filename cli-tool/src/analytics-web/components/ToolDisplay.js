...
class ToolDisplay {
  ...
  renderToolUse(toolBlock, toolResults = null) {
    ...
    const commandSummary = this.generateCompactCommand(
      toolName,
      toolBlock.input,
    );
    ...
    const contentId =
      originalToolName.toLowerCase() + "_" + toolId + "_" + Date.now();

    // Try to find corresponding tool result in toolResults first
    let matchingResult = null;
    if (toolResults && Array.isArray(toolResults)) {
      matchingResult = toolResults.find(
        (result) => result.tool_use_id === toolBlock.id,
      );
    }

    // Prepare comprehensive modal content for all tools (without tool results, as they're shown inline now)
    let modalContent = this.generateComprehensiveToolContent(
      toolName,
      toolBlock,
      null,
    );
    ...
    if (typeof window !== "undefined") {
      ...
      if (supportedTools.includes(originalToolName)) {
        console.log(
          `🔧 ToolDisplay: Storing ${originalToolName} tool data for contentId:`,
          contentId,
        );
    ...
    const buttonClass =
      toolName === "Bash" ? "bash-cmd-btn" : "tool-detail-btn";
  ...
  renderToolResultWithTruncation(toolResultBlock) {
    const toolId = toolResultBlock.tool_use_id
      ? toolResultBlock.tool_use_id.slice(-8)
      : "unknown";
  ...
  generateEnhancedResultContent(toolResultBlock) {
    ...
    if (
      toolResultBlock.returnCodeInterpretation &&
      toolResultBlock.returnCodeInterpretation !== "none"
    ) {
    ...
    if (
      toolResultBlock.stdout &&
      toolResultBlock.stdout !== toolResultBlock.content
    ) {
      if (content) content += "\n";
            ...
            for (let j = toolUseIndex; j < window.currentMessages.length; j++) {
              ...
              if (Array.isArray(laterMessage.content)) {
                for (const laterBlock of laterMessage.content) {
                  if (
                    laterBlock.type === "tool_result" &&
                    laterBlock.tool_use_id === toolUseId
                  ) {
  ...
  generateComprehensiveToolContent(toolName, toolBlock, resultContent) {
    ...
    switch (toolName) {
      ...
      case "Write":
        content += `File path: ${toolBlock.input?.file_path || "Unknown"}\n`;
        const contentLength = toolBlock.input?.content
          ? toolBlock.input.content.length
          : 0;
      ...
      case "Edit":
        ...
        const oldLength = toolBlock.input?.old_string
          ? toolBlock.input.old_string.length
          : 0;
        const newLength = toolBlock.input?.new_string
          ? toolBlock.input.new_string.length
          : 0;
      ...
      case "Grep":
        ...
        break;
      case "TodoWrite":
        const todoCount = Array.isArray(toolBlock.input?.todos)
          ? toolBlock.input.todos.length
          : 0;
