...
function displayCommandStats(analysis) {
  ...
  analysis.commands.forEach((cmd) => {
    ...
    const row =
      chalk.white(displayName.padEnd(nameWidth)) +
      chalk.gray(" │ ") +
  ...
  console.log(
    chalk.bold(
      `Total: ${analysis.total} commands, ${totalSizeKB} KB, ~${totalTokens} tokens`,
    ),
  );
...
async function promptSetupWhenNoCommands(targetDir) {
  ...
  console.log(
    chalk.gray(
      "No Claude Code commands found in this project. You can set up Claude Code Templates to get started.",
    ),
  );
  ...
  try {
    ...
    if (!setupNow) {
      console.log(
        chalk.yellow(
          '⏭️  Setup skipped. Run "npx claude-code-templates" anytime to set up your project.',
        ),
      );
      return false;
    }
    ...
    console.log(
      chalk.gray(
        "This will guide you through language and framework selection.\n",
      ),
    );
    ...
    console.log(
      chalk.blue(
        "💡 You can run setup manually with: npx claude-code-templates",
      ),
    );
    return false;
  }
...
async function promptCommandOptimization(analysis, targetDir) {
  ...
  console.log(
    chalk.gray(
      "Claude Code can analyze your commands and suggest improvements based on your project structure.",
    ),
  );
  ...
  try {
    const { optimize } = await inquirer.prompt([
      {
        ...
        name: "optimize",
        message:
          "Would you like Claude Code to review and optimize your commands?",
        default: true,
        prefix: chalk.blue("🤖"),
      },
    ]);
    ...
    if (!optimize) {
      console.log(
        chalk.yellow(
          "⏭️  Skipping optimization. You can run this anytime with --command-stats",
        ),
      );
      return;
    }
    ...
    console.log(
      chalk.gray(
        "This will analyze your project structure and suggest command improvements.\n",
      ),
    );
    ...
    claudeProcess.on("error", (error) => {
      if (error.code === "ENOENT") {
        ...
        console.log(
          chalk.blue(
            '💡 To run optimization manually later, use: claude "Analyze and optimize .claude/commands/ based on project structure"',
          ),
        );
      } else {
        console.error(
          chalk.red("Error running Claude Code optimization:"),
          error.message,
        );
      }
    });
    ...
    console.log(
      chalk.blue(
        '💡 You can run optimization manually with: claude "Analyze .claude/commands/ directory"',
      ),
    );
  }
}
...
function createOptimizationPrompt(analysis, targetDir) {
  const commandList = analysis.commands
    .map((cmd) => `- ${cmd.name}.md (${cmd.lines} lines)`)
    .join("\n");
