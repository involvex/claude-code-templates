...
function estimateTokens(text) {
  ...
  const adjustedTokens =
    baseTokens + codeBlocks * 5 + inlineCode * 2 + Math.ceil(markdownElements / 2);
...
function displayCommandStats(analysis) {
  ...
  const headerWidth =
    analysis.commands.length > 0
      ? Math.min(Math.max(...analysis.commands.map((cmd) => cmd.name.length)), 25) + 60
      : 80;
  ...
  console.log(chalk.gray('─'.repeat(separatorLength)));
  console.log(
    chalk.bold(`Total: ${analysis.total} commands, ${totalSizeKB} KB, ~${totalTokens} tokens`)
  );
...
async function promptCommandOptimization(analysis, targetDir) {
  ...
  try {
    ...
    if (!optimize) {
      console.log(
        chalk.yellow('⏭️  Skipping optimization. You can run this anytime with --command-stats')
      );
      return;
    }
    ...
    console.log(
      chalk.gray('This will analyze your project structure and suggest command improvements.\n')
    );
