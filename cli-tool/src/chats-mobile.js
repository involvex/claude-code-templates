      ...
      try {
        ...
        for (const conversation of this.data.conversations) {
          try {
            ...
            const parsedMessages = await this.conversationAnalyzer.getParsedConversation(
    ...
    this.app.get('/api/conversations/:id/analytics', async (req, res) => {
      try {
        ...
        const waitTimePercent =
          totalIterationTime > 0 ? Math.round((totalWaitTime / totalIterationTime) * 100) : 0;
        const userTimePercent =
          totalIterationTime > 0 ? Math.round((totalUserTime / totalIterationTime) * 100) : 0;
        ...
        const cacheWriteCost =
          ((conversation.tokenUsage?.cacheCreationTokens || 0) / 1000000) * 3.75;
        const cacheReadCost = ((conversation.tokenUsage?.cacheReadTokens || 0) / 1000000) * 0.3;
        ...
        if (messagesToBroadcast.length > 0) {
          ...
          for (const message of messagesToBroadcast) {
            if (this.webSocketServer) {
              ...
              const messageType =
                message.toolResults && message.toolResults.length > 0 ? 'tool' : 'text';
    ...
    return new Promise(async (resolve) => {
      this.httpServer = this.app.listen(this.port, async () => {
        ...
        try {
          ...
          this.log(
            'warn',
            chalk.yellow('⚠️  WebSocket server failed to initialize:', error.message)
          );
        }
