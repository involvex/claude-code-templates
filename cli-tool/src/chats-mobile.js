...
class ChatsMobile {
  ...
  setupMiddleware() {
    ...
    this.app.use(
      "/services",
      express.static(path.join(__dirname, "analytics-web", "services")),
    );
    this.app.use(
      "/components",
      express.static(path.join(__dirname, "analytics-web", "components")),
    );
    this.app.use(
      "/assets",
      express.static(path.join(__dirname, "analytics-web", "assets")),
    );
  }
  ...
  setupRoutes() {
    ...
    this.app.post("/api/search", async (req, res) => {
      try {
        const { query, workingDirectory, dateFrom, dateTo, contentSearch } =
          req.body;
        ...
        if (workingDirectory && workingDirectory.trim()) {
          results = results.filter((conv) => {
            ...
            return conv.project
              .toLowerCase()
              .includes(workingDirectory.toLowerCase());
          });
        }
        ...
        if (dateFrom) {
          ...
          results = results.filter(
            (conv) => new Date(conv.created) >= fromDate,
          );
        }
        ...
        if (contentSearch && contentSearch.trim()) {
          ...
          for (const conv of results) {
            try {
              ...
              this.log(
                "warn",
                `Error searching in conversation ${conv.id}:`,
                error.message,
              );
            }
          }
    ...
    this.app.post("/api/conversations/:id/download", async (req, res) => {
      try {
        ...
        console.log(
          chalk.cyan(
            `📥 Exporting conversation ${conversationId} as markdown...`,
          ),
        );

        // Export the session as markdown using SessionSharing module
        const exportResult = await this.sessionSharing.exportSessionAsMarkdown(
          conversationId,
          conversation,
        );
    ...
    this.app.get("/api/conversations/:id/analytics", async (req, res) => {
      try {
        ...
        console.log(
          chalk.cyan(
            `📊 Fetching analytics for conversation ${conversationId}...`,
          ),
        );

        // Get parsed messages for this conversation
        const messages = await this.conversationAnalyzer.getParsedConversation(
          conversation.filePath,
        );
        ...
        const waitTimePercent =
          totalIterationTime > 0
            ? Math.round((totalWaitTime / totalIterationTime) * 100)
            : 0;
        const userTimePercent =
          totalIterationTime > 0
            ? Math.round((totalUserTime / totalIterationTime) * 100)
            : 0;

        // Calculate cache efficiency
        const cacheTotal =
          (conversation.tokenUsage?.cacheCreationTokens || 0) +
          (conversation.tokenUsage?.cacheReadTokens || 0);
        const cacheEfficiency =
          cacheTotal > 0
            ? Math.round(
                ((conversation.tokenUsage?.cacheReadTokens || 0) / cacheTotal) *
                  100,
              )
            : 0;
        ...
        const inputCost =
          ((conversation.tokenUsage?.inputTokens || 0) / 1000000) * 3;
        const outputCost =
          ((conversation.tokenUsage?.outputTokens || 0) / 1000000) * 15;
        const cacheWriteCost =
          ((conversation.tokenUsage?.cacheCreationTokens || 0) / 1000000) *
          3.75;
        const cacheReadCost =
          ((conversation.tokenUsage?.cacheReadTokens || 0) / 1000000) * 0.3;
        const totalCost =
            ...
            messageContent.forEach((content) => {
              ...
              if (
                ...
                if (!componentsUsed.agents.find((a) => a.type === agentType)) {
                  ...
                  componentsUsed.agents.find((a) => a.type === agentType)
                    .count++;
                }
              }
              ...
              if (
                content.type === "tool_use" &&
                content.name === "SlashCommand" &&
                content.input?.command
              ) {
                const command = content.input.command;
                if (
                  !componentsUsed.slashCommands.find((c) => c.name === command)
                ) {
        ...
        if (cacheEfficiency < 20 && cacheTotal > 0) {
          optimizationTips.push(
            "• Low cache efficiency detected. Consider restructuring prompts to maximize cache reuse.",
          );
        }
        if (conversation.toolUsage?.totalToolCalls > 50) {
          optimizationTips.push(
            "• High tool usage detected. Review if all tool calls are necessary.",
          );
        }
        if (
          conversation.tokenUsage?.outputTokens >
          conversation.tokenUsage?.inputTokens * 2
        ) {
          optimizationTips.push(
            "• Output tokens significantly exceed input. Consider more concise prompts.",
          );
        }
        if (messages.length > 100) {
          optimizationTips.push(
            "• Long conversation detected. Consider starting fresh sessions for new topics to optimize context.",
          );
        }
        if (conversation.modelInfo?.hasMultipleModels) {
          optimizationTips.push(
            "• Multiple models used in this session. Stick to one model for consistency.",
          );
        }
        if (waitTimePercent > 70) {
          optimizationTips.push(
            "• High execution time detected. Consider breaking down complex tasks or optimizing tool usage.",
          );
        }
        if (optimizationTips.length === 0) {
          optimizationTips.push(
            "• Great! Your conversation shows efficient usage patterns.",
          );
        }
        ...
        const analytics = {
          ...
          modelInfo: {
            ...
            serviceTier:
              conversation.modelInfo?.currentServiceTier || "Unknown",
            hasMultipleModels:
              conversation.modelInfo?.hasMultipleModels || false,
          ...
          componentsUsed: {
            ...
            slashCommands: componentsUsed.slashCommands.sort(
              (a, b) => b.count - a.count,
            ),
  ...
  async setupFileWatching() {
    try {
      ...
      this.log(
        "warn",
        chalk.yellow("⚠️  File watching setup failed:", error.message),
      );
    }
  }
  ...
  async handleConversationChange(conversationId) {
    this.log(
      "info",
      chalk.gray(`💬 Conversation ${conversationId.slice(-8)} changed`),
    );
    ...
    try {
      ...
      if (parsedMessages && parsedMessages.length > 0) {
        ...
        const previousSnapshots =
          this.conversationMessageSnapshots.get(conversationId) || [];

        // Update the count
        this.conversationMessageCounts.set(conversationId, currentCount);

        // Generate current snapshots
        const currentSnapshots = parsedMessages.map((msg) =>
          this.generateMessageSnapshot(msg),
        );
        ...
        const newMessages =
          currentCount > previousCount
            ? parsedMessages.slice(previousCount)
            : [];
        ...
        for (let i = 0; i < Math.min(previousCount, currentCount); i++) {
          if (
            i < previousSnapshots.length &&
            currentSnapshots[i] !== previousSnapshots[i]
          ) {
            this.log("info", chalk.yellow(`🔄 Message ${i} changed:`));
            this.log(
              "info",
              chalk.gray(`   Previous: ${previousSnapshots[i]}`),
            );
            this.log("info", chalk.gray(`   Current:  ${currentSnapshots[i]}`));
            this.log(
              "info",
              chalk.gray(
                `   Message:  role=${parsedMessages[i].role}, content=${typeof parsedMessages[i].content}, toolResults=${parsedMessages[i].toolResults?.length || 0}`,
              ),
            );
        ...
        if (messagesToBroadcast.length > 0) {
          this.log(
            "info",
            chalk.cyan(
              `🔧 Found ${newMessages.length} new messages and ${updatedMessages.length} updated messages in conversation ${conversationId.slice(-8)}`,
            ),
          );
            ...
            if (this.webSocketServer) {
              ...
              const messageType =
                message.toolResults && message.toolResults.length > 0
                  ? "tool"
                  : "text";
              const toolCount = message.toolResults
                ? message.toolResults.length
                : 0;
              const hasToolsInContent =
                Array.isArray(message.content) &&
                message.content.some((block) => block.type === "tool_use");
              ...
              this.log(
                "info",
                chalk.cyan(
                  `🌐 Broadcasting ${isUpdatedMessage ? "updated" : "new"} ${messageType} message (${toolCount} tools) for ${conversationId.slice(-8)}`,
                ),
              );
              this.log(
                "info",
                chalk.gray(
                  `   Message details: role=${message.role}, hasToolResults=${!!message.toolResults}, hasToolsInContent=${hasToolsInContent}`,
                ),
              );
              if (message.toolResults) {
                this.log(
                  "info",
                  chalk.gray(
                    `   Tool results: ${message.toolResults.map((tr) => tr.tool_use_id || "no-id").join(", ")}`,
                  ),
                );
              }
          ...
          console.log(
            chalk.gray(
              `📝 No new messages in conversation ${conversationId.slice(-8)} (${currentCount} total)`,
            ),
          );
        }
      }
      ...
      console.warn(
        chalk.yellow("⚠️  Error handling conversation change:", error.message),
      );
    }
  }
  ...
  getMessagePreview(text, searchTerm, contextLength = 100) {
    ...
    const end = Math.min(
      text.length,
      position + searchTerm.length + contextLength / 2,
    );
  ...
  async loadInitialData() {
    try {
      ...
      if (await fs.pathExists(claudeDataDir)) {
        ...
        const conversations = await this.conversationAnalyzer.loadConversations(
        ...
        for (const conversation of conversations) {
          try {
            ...
            const snapshots = parsedMessages.map((msg) =>
              this.generateMessageSnapshot(msg),
            );
        ...
        console.log(
          chalk.green(
            `📂 Loaded ${this.data.conversations.length} conversations`,
          ),
        );
        console.log(
          chalk.gray(
            `📊 Initialized message counts for ${this.conversationMessageCounts.size} conversations`,
          ),
        );
      } else {
      ...
      console.warn(
        chalk.yellow("⚠️  Failed to load initial data:", error.message),
      );
    }
  }
  ...
  async startServer() {
    return new Promise(async (resolve) => {
      this.httpServer = this.app.listen(this.port, async () => {
        ...
        console.log(
          chalk.green(`📱 Chats Mobile server started at ${this.localUrl}`),
        );
        ...
        try {
          ...
          this.log(
            "warn",
            chalk.yellow(
              "⚠️  WebSocket server failed to initialize:",
              error.message,
            ),
          );
        }
  ...
  async setupCloudflaredTunnel() {
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        cloudflared.stdout.on("data", (data) => {
          ...
          if (!urlMatch) {
            ...
            urlMatch = str.match(
              /https:\/\/[a-zA-Z0-9.-]+\.(?:trycloudflare|cfargotunnel)\.com/,
            );
          }
          ...
          if (urlMatch) {
            ...
            console.log(
              chalk.green(`☁️  Cloudflare Tunnel ready: ${this.tunnelUrl}`),
            );
        ...
        cloudflared.on("error", (error) => {
          console.error(
            chalk.red("❌ Failed to start Cloudflare Tunnel:"),
            error.message,
          );
          console.log(
            chalk.yellow(
              "💡 Make sure cloudflared is installed: https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/",
            ),
          );
          resolve(null);
        });
        ...
        cloudflared.on("close", (code) => {
          console.log(
            chalk.yellow(`⚠️  Cloudflared process exited with code ${code}`),
          );
          if (!this.tunnelUrl) {
            resolve(null);
          }
        ...
        setTimeout(() => {
          if (!this.tunnelUrl) {
            console.warn(
              chalk.yellow("⚠️  Tunnel URL not detected within 45 seconds"),
            );
            console.log(chalk.gray("Full cloudflared output:"));
            console.log(chalk.gray(output));
            console.log(
              chalk.blue("💡 You can manually run: ") +
                chalk.white(`cloudflared tunnel --url ${this.localUrl}`),
            );
            console.log(
              chalk.blue(
                "   Then copy the tunnel URL and access it in your browser.",
              ),
            );
            resolve(null);
          }
      ...
      console.error(
        chalk.red("❌ Error setting up Cloudflare Tunnel:"),
        error.message,
      );
      return null;
    }
  ...
  async openBrowser() {
    try {
      ...
      const url =
        this.tunnelUrl || this.localUrl || `http://localhost:${this.port}`;
      console.log(chalk.cyan(`🌐 Opening browser to ${url}`));
      await open(url);
    } catch (error) {
      console.warn(
        chalk.yellow("⚠️  Could not auto-open browser:", error.message),
      );
    }
  }
  ...
  async stop() {
    ...
    if (this.cloudflaredProcess) {
      try {
        ...
        this.log(
          "warn",
          chalk.yellow("⚠️  Error stopping Cloudflare Tunnel:", error.message),
        );
      }
    }
    ...
    if (this.webSocketServer) {
      try {
        ...
        this.log(
          "warn",
          chalk.yellow("⚠️  Error stopping WebSocket server:", error.message),
        );
      }
    }
...
async function startChatsMobile(options = {}) {
  ...
  try {
    ...
    if (chatsMobile.tunnelUrl) {
      ...
      console.log(
        chalk.blue(`🌐 Opening remote URL: ${chatsMobile.tunnelUrl}`),
      );
    }
