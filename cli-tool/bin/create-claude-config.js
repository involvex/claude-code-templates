...
function showBanner() {
  ...
  console.log(
    chalk.hex('#D97706')('🚀 Setup Claude Code for any project language 🚀') +
      chalk.gray(`\n                             v${pkg.version}\n\n`) +
      chalk.blue('🌐 Templates: ') +
      chalk.underline('https://aitmpl.com') +
      '\n' +
      chalk.blue('📖 Documentation: ') +
      chalk.underline('https://docs.aitmpl.com') +
      '\n'
  );
}
...
program
  ...
  .description(
    'Setup Claude Code configurations and create global AI agents powered by Claude Code SDK'
  )
  ...
  .option('--dashboard', 'launch unified dashboard with all tools (default: analytics view)')
  ...
  .option('--hooks', 'launch Hooks Dashboard to view and manage automation hooks')
  .option('--host <host>', 'host for dashboard server (default: localhost)')
  .option('--port <port>', 'port for dashboard server (default: auto)')
  .option(
    '--tunnel',
    'enable Cloudflare Tunnel for remote access (use with --analytics or --chats)'
  )
  .option('--verbose', 'enable verbose logging for debugging and development')
  .option(
    '--health-check, --health, --check, --verify',
  ...
  .option(
    '--workflow <workflow>',
    'install workflow from hash (#hash) OR workflow YAML (base64 encoded) when used with --agent/--command/--mcp'
  )
  .option(
    '--prompt <prompt>',
  ...
  .option(
    ...
    'Set the Scope where the Configuration will be applied'
  )
  .action(async (options) => {
    try {
      // Only show banner for non-agent-list commands
      const isQuietCommand = options.listAgents || options.removeAgent || options.updateAgent;
