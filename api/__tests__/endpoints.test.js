...
describe('API Endpoints - Critical Tests', () => {
  describe('🔴 CRITICAL: Component Download Tracking', () => {
    test(
      'POST /api/track-download-supabase should be available',
      async () => {
    ...
    test(
      'POST /api/track-download-supabase should reject requests without data',
      async () => {
        const response = await axios.post(
          `${BASE_URL}/api/track-download-supabase`,
          {},
    ...
    test(
      'POST /api/track-download-supabase should validate component type',
      async () => {
        const response = await axios.post(
          `${BASE_URL}/api/track-download-supabase`,
          {
  ...
  describe('🟡 Discord Bot Integration', () => {
    test(
      'POST /api/discord/interactions should be available',
      async () => {
        const response = await axios.post(
  ...
  describe('🟢 Claude Code Changelog Monitor', () => {
    test(
      'GET /api/claude-code-check should be available',
      async () => {
        const response = await axios.get(`${BASE_URL}/api/claude-code-check`, {
          timeout: TIMEOUT,
          validateStatus: () => true,
        });
  ...
  describe('🔵 Command Usage Tracking', () => {
    test(
      'POST /api/track-command-usage should be available',
      async () => {
        const response = await axios.post(
    ...
    test(
      'POST /api/track-command-usage should reject invalid commands',
      async () => {
        const response = await axios.post(
          `${BASE_URL}/api/track-command-usage`,
          {
    ...
    test(
      'POST /api/track-command-usage should reject requests without command',
      async () => {
        const response = await axios.post(
          `${BASE_URL}/api/track-command-usage`,
          {
  ...
  describe('📊 API Health Check', () => {
    test('All critical endpoints should respond within 30s', async () => {
      ...
      for (const endpoint of endpoints) {
        const method = endpoint.includes('track-') || endpoint.includes('discord') ? 'post' : 'get';

        try {
          await axios[method](`${BASE_URL}${endpoint}`, method === 'post' ? {} : undefined, {
            timeout: TIMEOUT,
            validateStatus: () => true,
          });
        } catch (error) {
...
describe('API Endpoints - Functional Tests', () => {
  describe('Component Download Tracking - Data Validation', () => {
    ...
    test(
      'should accept all valid component types',
      async () => {
  ...
  describe('Claude Code Monitor - Parser Tests', () => {
    test(
      'GET /api/claude-code-check should return valid structure',
      async () => {
        const response = await axios.get(`${BASE_URL}/api/claude-code-check`, {
          timeout: TIMEOUT,
          validateStatus: () => true,
  ...
  describe('Command Usage Tracking - Data Validation', () => {
    ...
    test(
      'should accept all valid commands',
      async () => {
...
describe('API Security & CORS', () => {
  test(
    'endpoints should have correct CORS headers',
    async () => {
      const response = await axios.options(`${BASE_URL}/api/track-download-supabase`, {
        timeout: TIMEOUT,
        validateStatus: () => true,
      });
  ...
  test(
    'endpoints should handle incorrect HTTP methods',
    async () => {
      const response = await axios.get(`${BASE_URL}/api/track-download-supabase`, {
        timeout: TIMEOUT,
        validateStatus: () => true,
      });
