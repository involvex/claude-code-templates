...
class PluginPageManager {
  ...
  renderPluginDetails() {
    ...
    document.getElementById('installPluginCmd').textContent =
  ...
  renderComponents(plugin) {
    ...
    if (plugin.mcpServers && plugin.mcpServers > 0) {
      ...
      mcpsList.innerHTML = this.generateComponentItems(plugin.mcpServersList || [], 'mcp', 'mcps');
    }
  }
  ...
  renderRelatedPlugins() {
    ...
    if (relatedPlugins.length === 0) {
      container.innerHTML =
        '<p style="color: var(--text-secondary);">No related plugins found.</p>';
      return;
    }
