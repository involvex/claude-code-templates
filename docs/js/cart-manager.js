...
class CartManager {
  ...
  cleanCorruptedData() {
    ...
    Object.keys(this.cart).forEach((type) => {
      if (Array.isArray(this.cart[type])) {
        ...
        this.cart[type] = this.cart[type].filter(
          (item) => item && typeof item === 'object' && item.name && item.path
        );
  ...
  updateCommand() {
    ...
    if (this.cart.settings.length > 0) {
      const settingsPaths = this.cart.settings
        .map((item) => this.getCleanPath(item.path))
        .join(',');
      ...
      for (const mutation of mutations) {
        ...
        if (
          ...
          Array.from(mutation.addedNodes).some(
            (node) =>
              node.nodeType === 1 &&
              (node.classList?.contains('unified-card') || node.querySelector?.('.unified-card'))
          )
        ) {
          shouldUpdate = true;
          break;
...
function downloadStack() {
  ...
  if (shouldExecute) {
    ...
    cartManager.showNotification(
      'Command copied! Paste it in your terminal to download the stack.',
      'success'
    );
