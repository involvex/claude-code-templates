...
class WorkflowManager {
    ...
    if (agents.length > 0) {
      ...
      agents.forEach((agentData, index) => {
        ...
        if (agentData.subItems === null) {
          ...
          const subItemsHtml = agentData.subItems
            .map(
              (subItem, subIndex) => `
  ...
  async generateWorkflowHash() {
    ...
    const agents = this.workflowState.steps
      .filter((step) => step.type === 'agent')
      .map((step) => step.path.replace(/\.md$/, ''));
    const commands = this.workflowState.steps
      .filter((step) => step.type === 'command')
      .map((step) => step.path.replace(/\.md$/, ''));
    const mcps = this.workflowState.steps
      .filter((step) => step.type === 'mcp')
      .map((step) => step.path.replace(/\.json$/, ''));
    ...
    const componentsOnlyCommand =
      componentsParts.length > 0
  ...
  generateWorkflowYAML() {
    ...
    const totalSteps = agents.reduce(
      (total, agent) => total + 1 + (agent.subItems ? agent.subItems.length : 0),
      0
    );
...
${agents
  .map(
    ...
    tasks:${
      agentData.subItems && agentData.subItems.length > 0
        ? agentData.subItems
            .map(
              (subItem, j) => `
      - task: ${j + 1}
      ...
      - task: 1
        ...
        description: "This agent has no specific commands or MCPs assigned"`
    }`
  )
  .join('\n')}
...
components:
  agents: [${agents
    .filter((a) => a.agent.type === 'agent')
    .map((a) => `".claude/agents/${a.agent.path}"`)
    .join(', ')}]
...
# Each agent should be loaded first, then its associated commands and MCPs should be executed in sequence.`;
  ...
  showGenerateModal(commandData, yaml) {
    ...
    if (copyComponentsCommandBtn) {
      copyComponentsCommandBtn.onclick = () =>
        copyToClipboard(componentsCommand, 'Components command copied!');
    }
  ...
  showPreviewModal(yamlContent) {
    ...
    lineNumbersElement.innerHTML = lines.map((_, index) => `<span>${index + 1}</span>`).join('');
...
function removeWorkflowStep(stepId) {
  if (window.workflowManager) {
    window.workflowManager.workflowState.steps = window.workflowManager.workflowState.steps.filter(
      (step) => step.id !== stepId
    );
