...
class ComponentPageManager {
    ...
    if (validationChecks && security.validators) {
      ...
      Object.entries(security.validators).forEach(([key, validatorData]) => {
        ...
        const hasErrors =
          ...
          validatorData.errors.length > 0;
        const hasWarnings =
          typeof validatorData === 'object' &&
          validatorData.warnings &&
          validatorData.warnings.length > 0;
        const hasInfo =
          typeof validatorData === 'object' && validatorData.info && validatorData.info.length > 0;
  ...
  async renderMetadataSection() {
    ...
    try {
      ...
      console.log(
        'Available agents:',
        marketplace.agents.map((a) => a.name)
      );

      const agentMetadata = marketplace.agents.find((agent) => agent.name === componentName);
  ...
  async loadComponentsMarketplace() {
    try {
      ...
      const marketplaceResponse = await fetch(
        'https://raw.githubusercontent.com/davila7/claude-code-templates/main/cli-tool/components/.claude-plugin/marketplace.json'
      );
  ...
  async renderComponentCode() {
    try {
      ...
      const lineNumbers = lines
        .map((_, index) => {
          ...
          if (errorLines.has(lineNum)) {
            ...
            style =
              'background-color: rgba(245, 101, 101, 0.15); color: #f56565; font-weight: 600; border-left: 3px solid #f56565; padding-left: 5px; cursor: pointer;';
            title = 'Line has validation errors - Click to see details';
          } else if (warningLines.has(lineNum)) {
            className = 'line-with-warning';
            style =
              'background-color: rgba(237, 137, 54, 0.15); color: #ed8936; font-weight: 600; border-left: 3px solid #ed8936; padding-left: 5px; cursor: pointer;';
      ...
      if (lineNumbersElement) {
        ...
        const clickableLines = lineNumbersElement.querySelectorAll(
          '.line-with-error, .line-with-warning'
        );
        clickableLines.forEach((lineSpan) => {
          lineSpan.addEventListener('click', () => {
            ...
            if (this.component.security && this.component.security.validators) {
              ...
              for (const [validatorKey, validatorData] of Object.entries(
                this.component.security.validators
              )) {
                if (typeof validatorData === 'object') {
                  // Check if this validator has errors/warnings for this line
                  const hasError =
                    validatorData.errors &&
                    validatorData.errors.some((error) => {
                      if (error.line === lineNum) return true;
                      if (error.examples && error.examples.some((ex) => ex.line === lineNum))
                        return true;
                      return false;
                    });
                  ...
                  const hasWarning =
                    validatorData.warnings &&
                    validatorData.warnings.some((warning) => {
                      if (warning.line === lineNum) return true;
                      if (warning.examples && warning.examples.some((ex) => ex.line === lineNum))
                        return true;
                      return false;
                    });
                  ...
                  if (hasError || hasWarning) {
                    ...
                    const checkDescriptions = {
                      structural:
                        'Verifies file format, YAML frontmatter, required fields, and encoding',
                      integrity: 'Checks for tampering using SHA256 hash and version tracking',
                      semantic:
                        'Detects malicious patterns, prompt injection, and dangerous commands',
                      references: 'Validates external URLs and prevents SSRF attacks',
                      provenance: 'Confirms author metadata and repository information',
                    };
      ...
      addToCartBtn.addEventListener('click', (e) => {
        ...
        if (this.component) {
          ...
          if (typeof addToCart === 'function') {
            ...
            alert(
              'Cart functionality not available. Please return to the main page to add components to your stack.'
            );
          }
        } else {
  ...
  updatePageMetadata() {
    ...
    const typeCapitalized =
      this.component.type.charAt(0).toUpperCase() + this.component.type.slice(1);
...
function shareComponentOnTwitter() {
  ...
  if (componentManager && componentManager.component) {
    ...
    const type =
      componentManager.component.type.charAt(0).toUpperCase() +
      componentManager.component.type.slice(1);
...
#ClaudeCode #AI #Development #${category.replace(/\s+/g, '')} #Automation`;
    ...
    const componentTitle =
      document.getElementById('componentTitle')?.textContent || 'Claude Code Component';
...
window.openValidationModal = function (validatorKey, validatorLabel, validatorData, description) {
  ...
  const passed = typeof validatorData === 'boolean' ? validatorData : validatorData.valid;
  const hasErrors =
    typeof validatorData === 'object' && validatorData.errors && validatorData.errors.length > 0;
  const hasWarnings =
    typeof validatorData === 'object' &&
    validatorData.warnings &&
    validatorData.warnings.length > 0;
