...
class BlogLoader {
  ...
  sortArticles(sortBy) {
    ...
    switch (sortBy) {
      case 'date-asc':
        this.filteredArticles.sort((a, b) => parseDate(a.publishDate) - parseDate(b.publishDate));
        break;
      case 'difficulty-asc':
      ...
      default:
        ...
        this.filteredArticles.sort((a, b) => parseDate(b.publishDate) - parseDate(a.publishDate));
        break;
    }
  }
  ...
  async loadArticles() {
    try {
      ...
      console.log(
        `Loaded ${this.articles.length} articles (${this.featuredArticles.length} featured)`
      );
    } catch (error) {
